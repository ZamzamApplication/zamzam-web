...
function SheikhAccordion({
  ...
  return (
    <div className="attendance-sheikh-card glass-card relative z-0 overflow-visible rounded-2xl">
      <button
        onClick={onToggle}
...
export default function SessionAttendancePage() {
  ...
  const handleProgressChange = useCallback((draft: QuranProgressInput) => {
    ...
    setProgressDrafts((current) => ({ ...current, [key]: draft }))
  ...
  const saveProgressDrafts = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      setProgressCategories(refreshed.categories?.length ? refreshed.categories : ['new_memorization', 'recent_revision'])
