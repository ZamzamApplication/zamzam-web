...
export default function SubscriptionsPage() {
  ...
  const [sheikhs, setSheikhs] = useState<{ id: number; name: string }[]>([])
  ...
  useEffect(() => {
    ...
    async function loadInitial() {
      try {
        ...
        const nextSettings = await api.getSubscriptionSettings()
        const nextFinanceSettings = await api.getFinanceSettings()
        const [nextSheikhs, expenseCategories] = user.role === 'auditor'
          ? [nextFinanceSettings.sheikhs, nextFinanceSettings.expense_categories]
          : await Promise.all([api.getSheikhs(), api.getTahfizSettings().then(value => value.expense_categories || [])])
        if (cancelled) return
  ...
  return <div className="space-y-5">
    <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
      <div>{!isAuditor && <Link href="/manage" className="text-sm font-semibold text-blue-700 dark:text-blue-300">الإدارة ‹</Link>}<h1 className="mt-1 text-2xl font-bold text-deep-900">القسم المالي</h1><p className="mt-1 text-sm text-deep-500">الاشتراكات والمصروفات والوضع المالي للتحفيظ{isAuditor ? ' — صلاحية عرض فقط' : ''}</p></div>
      {!isAuditor && <div className="flex flex-wrap gap-2"><button type="button" onClick={() => { setActiveSection('expenses'); setEditingExpense(null) }} disabled={expenseCategories.every(category => !category.enabled)} className="water-btn rounded-xl px-4 py-2.5 text-sm font-bold text-white disabled:opacity-50">إضافة مصروف</button><button type="button" onClick={() => setShowSettings(true)} className="water-btn-outline rounded-xl px-4 py-2.5 text-sm font-semibold">إعدادات المالية</button></div>}
    </header>
    ...
    {!isAuditor && selected.size > 0 && <div className="sticky top-3 z-20 flex flex-col gap-3 rounded-2xl border border-cyan-300 bg-cyan-50/95 p-4 shadow-lg backdrop-blur sm:flex-row sm:items-center sm:justify-between dark:border-cyan-800 dark:bg-cyan-950/95">
    ...
    <section className="glass-card overflow-hidden rounded-2xl">
      <div className="flex items-center justify-between border-b border-water-200 p-4"><label className="flex items-center gap-2 text-sm text-deep-700">{!isAuditor && <><input type="checkbox" checked={allVisibleSelected} onChange={() => setSelected(allVisibleSelected ? new Set() : new Set(unpaidOnPage.map(record => record.id)))} /> تحديد غير المدفوع في الصفحة</>}</label><span className="text-xs text-deep-500">{total} سجل</span></div>
      {listLoading ? <div className="page-loading my-12" aria-label="جاري تحميل السجلات" /> : records.length === 0 ? <div className="p-12 text-center text-deep-500">لا توجد اشتراكات مطابقة لهذه الفلاتر.</div> : <div className="divide-y divide-water-200/60">
        {records.map(record => <article key={record.id} className="grid gap-3 p-4 hover:bg-water-50/40 md:grid-cols-[auto_1.4fr_1fr_1fr_auto] md:items-center">
          {!isAuditor && <input type="checkbox" aria-label={`تحديد ${record.student_name}`} disabled={record.is_paid || record.fee_minor === 0} checked={selected.has(record.id)} onChange={() => setSelected(previous => { const next = new Set(previous); next.has(record.id) ? next.delete(record.id) : next.add(record.id); return next })} className="h-4 w-4" />}
          ...
          <div className="flex flex-wrap gap-2 md:justify-end">
            {isAuditor ? (record.is_paid && <button type="button" disabled={busy} onClick={() => void showReceipt(record)} className="water-btn-outline rounded-lg px-3 py-1.5 text-xs">الإيصال</button>) : <>
            ...
            {!isAuditor && record.student_id !== null && <Link href={`/students/${record.student_id}`} className="rounded-lg px-2 py-1.5 text-xs font-semibold text-blue-700 dark:text-blue-300">ملف الطالب</Link>}
          </div>
        </article>)}
      </div>}
    </section>
